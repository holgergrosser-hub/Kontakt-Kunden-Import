...
|---|---|---|---|---|
| E20 | **Ist die Kundenverwaltung vorhanden?** (Zusatz 18.09.2026: Billomat exportiert Kunden- und Rechnungsdaten per API; damit ist Billomat die Startquelle für den Kundenstamm, siehe Runde 2 Kap. 4.8.) Der Auftragstext („Mir fehlt die Verwaltung der Kunden habe ich") ist doppeldeutig. | Vorhanden: CRM-Sheet aus Kontakt-Kunden-Import ist Kundenstamm, Zertifikatssystem referenziert per K-Nr. | Fehlt: Kundenverwaltung ist Kern der Anforderung. | **Entschieden (18.09.2026):** Das CRM-System ist der einzige Kundenstamm für Beratung und Zertifizierung; Kunden werden nur einmal erfasst. `Kunden` in der Mandantendatei ist nur Referenz (K-Nr., Billomat-ID, Portal-Token), kein zweiter Stamm. Siehe M1.11 bis M1.13. |
